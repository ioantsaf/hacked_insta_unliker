...
def display_exclude_menu():
    while True:
        username = input(
            "Write a username to exclude. Leave empty to stop excluding usernames:\n"
        )
...
def exclude_likes():
    suspicious_likes[:] = [
        l for l in suspicious_likes if get_username(l) not in excluded_users
    ]
...
def display_menu(client):
    ...
    if operation == "1":
        operation = (
            input(
                f"Are you sure you wish to remove {len(suspicious_likes)} likes from the above users? [(y)es/(n)o]: "
            )
            .strip()
            .lower()
        )
...
def main():
    ...
    while process_next_page:
        (process_next_page, next_max_id) = process_like_page(
            client, date_threshold, next_max_id
        )
